...
if ( get_option( 'trp_license_key' ) != '123456-123456-123456-123456' ) {
	update_option( 'trp_license_key', '123456-123456-123456-123456' );
}

$trpLicenseDetails = get_option( 'trp_license_details' );
if ( array_key_exists( 'invalid', $trpLicenseDetails ) || ! array_key_exists( 'valid', $trpLicenseDetails ) ) {
	$license_information_for_all_addons            = [];
	$license_information_for_all_addons['valid'][] = true;
	update_option( 'trp_license_details', $license_information_for_all_addons );
}
...
if ( ! class_exists( 'TRP_Handle_Included_Addons' ) ) {
	class TRP_Handle_Included_Addons {
		...
		function __construct() {
			...
			add_action( 'trp_add_ons_activate', array(
				$this,
				'trp_activate_add_ons'
			) );
			//deactivate an add-on when you press the button from the add-ons page
			add_action( 'trp_add_ons_deactivate', array(
				$this,
				'trp_deactivate_add_ons'
			) );
			//show the button in the add-ons page with the correct action
			add_filter( 'trp_add_on_is_active', array(
				$this,
				'trp_check_add_ons_activation'
			), 10, 2 );

			add_action( 'admin_notices', array(
				$this,
				'trp_main_plugin_notice'
			) );
			...
			if ( ! defined( 'TRP_PLUGIN_VERSION' ) ) {
				if ( $tp_installation_status === 'no_action_requested' ) {
					...
					echo '<p><a href="' . esc_url( add_query_arg( array(
							'action' => 'trp_install_tp_plugin',
							'nonce'  => wp_create_nonce( 'trp_install_tp_plugin' )
						) ) ) . '" type="button" class="button-primary">' . esc_html__( 'Install & Activate', 'translatepress-multilingual' ) . '</a></p>';
					echo '</div>';
				}
			} else {
		...
		function disable_old_add_ons() {
			...
			if ( isset( $_GET['activate'] ) && $deactivated_addons === 1 ) {
				...
				add_action( 'admin_notices', array(
					$this,
					'disable_old_add_ons_notice_multi'
				) );
			}
			...
			if ( ! empty( $old_add_ons_status ) ) {
				...
				if ( empty( $old_add_ons_option ) ) {
					update_option( 'trp_old_add_ons_status', $old_add_ons_status );
...
function trp_developer_update() {
	...
	if ( class_exists( 'TRP_EDD_SL_Plugin_Updater' ) ) {
		...
		$edd_updater = new TRP_EDD_SL_Plugin_Updater( TRP_DEVELOPER_STORE_URL, __FILE__, array(
				...
				'item_name' => TRP_DEVELOPER_ITEM_NAME,
				// name of this plugin
				'item_id'   => '224',
				'author'    => 'Cozmoslabs',
